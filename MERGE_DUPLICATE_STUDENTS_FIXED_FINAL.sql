-- MERGE: Combine subjects from duplicate student registrations - CORRECTED FOR TEXT[] TYPE
-- Handles subjects stored as text[] (text array), not JSONB
-- Keep the NEWEST record, merge all subjects, delete the OLDER ones

-- ========================================
-- STEP 0: First, identify duplicate students
-- ========================================
...
CREATE TABLE students_merge_backup_final AS
WITH duplicate_groups AS (
  SELECT
    s.institute_id,
    s.name
...
-- Verify backup
SELECT COUNT(*) as total_records_backed_up FROM students_merge_backup_final;
...
-- NOTE: Handles subjects as text[] array type
-- ========================================
...
all_subjects_combined AS (
  ...
  FROM duplicate_groups dg
  JOIN public.students s ON dg.institute_id = s.institute_id AND dg.name = s.name,
  LATERAL unnest(COALESCE(s.subjects, ARRAY[]::text[])) as subject_elem
...
merged_subjects_per_group AS (
  SELECT
    ...
    (SELECT array_agg(DISTINCT elem ORDER BY elem)
     FROM (
       SELECT DISTINCT unnest(COALESCE(s.subjects, ARRAY[]::text[])) as elem
       FROM duplicate_groups dg
       JOIN public.students s ON dg.institute_id = s.institute_id AND dg.name = s.name
...
FROM students_merge_backup_final
UNION ALL
SELECT
...
-- DROP TABLE students_merge_backup_final;
