-- Admin forgot password: invite/profile email lookup + server-staged OTP for reset edge function.
...
create table if not exists public.admin_password_reset_otps (
  ...
  primary key (institute_id, email)
);
...
-- Email shown on forgot-password screen: prefer website [admin_invites], else active admin profile.
create or replace function public.get_admin_reset_email_for_institute(p_key text)
returns text
language sql
stable
security definer
...
as $$
  with inst as (
    ...
    where trim(i.id::text) = trim(p_key)
       or trim(coalesce(i.institute_code, '')) = trim(p_key)
    limit 1
  )
  select coalesce(
    (
      select trim(ai.email::text)
      from public.admin_invites ai
      join inst on inst.institute_id = ai.institute_id
    ...
    (
      ...
      join inst on inst.institute_id = p.institute_id
      where p.role = 'admin'
...
grant execute on function public.get_admin_reset_email_for_institute(text) to anon, authenticated;

-- Stage OTP (called from app before Brevo email-otp send).
...
grant execute on function public.stage_admin_password_reset_otp(text, text, text) to anon, authenticated;

-- Verify OTP and return profile id for auth password update (edge function).
...
grant execute on function public.consume_admin_password_reset_otp(text, text, text) to anon, authenticated;
